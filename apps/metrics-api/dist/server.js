...
const INGEST_KEYS = [process.env.METRICS_INGEST_KEY, process.env.NEXT_METRICS_INGEST_KEY].filter((value) => Boolean(value && value.trim()));
...
export function observeCheckinStage(stage, seconds, channel = 'kiosk') {
...
if (INGEST_KEYS.length > 0) {
    app.use('/observe', (req, res, next) => {
        const provided = req.get('x-metrics-key') ?? req.query.key;
...
app.post('/observe/checkin-stage', (req, res) => {
...
app.post('/observe/assign', (req, res) => {
