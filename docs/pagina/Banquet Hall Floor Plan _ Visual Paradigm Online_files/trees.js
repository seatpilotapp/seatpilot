...
Draw.loadPlugin(function(ui)
{
	if (ui.editor.isChromelessView())
	{
        ...
        var createTraverseTreeVerticesAndEdgesWrapperFunction = function(actualFunction) {
            return function(vertex, edge) {
                var result = false;
                
                if (edge == null || vertex.isTreeStructureEdge(edge)) {
                    result = actualFunction(vertex, edge);
                }
	...
	function isTreeCell(cell)
	{
		...
		if (cell != null)
		{
			graph.traverse(cell, true, createTraverseTreeVerticesAndEdgesWrapperFunction(function(vertex)
			{
				result = vertex.isTreeRoot();
	...
	ui.actions.addAction('selectSubtree', function()
	{
		if (graph.isEnabled() && graph.getSelectionCount() == 1)
		{
			...
			graph.traverse(cell, true, createTraverseTreeVerticesAndEdgesWrapperFunction(function(vertex, edge)
			{
				if (edge != null)
				{
					subtree.push(edge);
	...
	graph.foldCells = function(collapse, recurse, cells, checkFoldable, evt)
	{
		...
		try
		{
			...
			for (var i = 0; i < cells.length; i++)
			{
				...
				{
					graph.traverse(cells[i], true, createTraverseTreeVerticesAndEdgesWrapperFunction(function(vertex, edge)
					{
						if (edge != null)
						{
							tmp.push(edge);
	...
	graph.duplicateCells = function(cells, append)
	{
		...
		try
		{
			...
			{
                                var calculateClonedIdCellMap = function(cells, clonedCells) {
                                    var result = {};
                                    
                                    // Caller already ensured cells and clonedCells not null and of same length.
                                    var count = cells.length;
                                    
                                    for (var i = 0; i < count; i++) {
                                        var eachCell = cells[i];
                                        var eachClonedCell = clonedCells[i];                                        
                                        
                                        if (eachCell != null && eachClonedCell != null) {
                                            var cellId = eachCell.getId();
                                            result[cellId] = eachClonedCell;
                                        }
                                    }                        
                                ...
                                var validateEdgeSourceConsiderClonedSource = function(edgeSource, clonedIdCellMap) {
                                    if (edgeSource != null) {
                                        var edgeSourceId = edgeSource.getId();                                        
                                        var clonedEdgeSource = clonedIdCellMap[edgeSourceId];

                                        if (clonedEdgeSource != null) {
                                            edgeSource = clonedEdgeSource;
                                        }
                                    }
                                ...
                                var clonedIdCellMap = calculateClonedIdCellMap(cells, result);
					...
					if (isTreeCell(cells[i]))
					{
						...
						if (newEdges.length == 0 && edges.length > 0)
						{
							var clone = this.cloneCells([edges[0]])[0];
                                                        ...
                                                        var edgeSource = this.model.getTerminal(edges[0], true);
                                                        edgeSource = validateEdgeSourceConsiderClonedSource(edgeSource, clonedIdCellMap);
                                                        
                                                        var edgeTarget = result[i];
                                                                
							this.addEdge(clone, graph.getDefaultParent(),
								edgeSource, edgeTarget);
                                                                
                                                        // Quick fix for mind mapping node show wrong level appearance.        
                                                        if (edgeTarget != null && edgeTarget.getShapeType() === 'MindMapNode') {
                                                            graph.redrawCell(edgeTarget);
                                                        }
						}
					}
				}
	...
	graph.moveCells = function(cells, dx, dy, clone, target, evt, mapping)
	{
		var result = null;
                ...
                // May null if import an empty diagram XML from device.
                if (cells != null &&
                    cells.length > 0) {
	...
	if (ui.sidebars != null) {
		for (var paletteTab in this.sidebars) {
			if (this.sidebars[paletteTab] != null) {
	...
	function addSibling(cell, after)
	{
		...
		try
		{
						...
						if (dir == getTreeDirection(temp))
						{
							...
							if (temp != cell && sibling != null)
							{
									...
									(!hor && after != sibling.getCenterY() < state.getCenterY()))
								{
									...
									graph.traverse(sibling.cell, true, createTraverseTreeVerticesAndEdgesWrapperFunction(function(vertex, edge)
									{
										if (edge != null)
										{
											subtree.push(edge);
			...
			var result = graph.addCells(clones);
                        
                        clones[1].fireTreeNodeClonedAsSiblingOfTreeNode(graph, cell, clones[0]);
	...
	function addParent(cell)
	{
		...
		try
		{
			...
			graph.moveCells(subtree, dx, dy);
                        ...
                        clones[1].fireTreeNodeClonedAsParentOfTreeNode(graph, cell, clones[0]);
	...
	function addChild(cell)
	{
		...
		try
		{
			...
			var result = graph.addCells(clones);
                        ...
                        clones[1].fireTreeNodeClonedAsChildOfTreeNode(graph, cell, clones[0]);
	...
	ui.onKeyDown = function(evt)
	{
		try
		{
				...
				isTreeCell(graph.getSelectionCell()))
			{
				...
				if (graph.getSelectionCell().isTreeRoot() === false)
				{
	...
	{
		if (isTreeCell(source) && source.isTreeRoot() === false)
		{
		...
		else
		{
			...
			try
			{
				var cells = graphConnectVertex.call(this, source, direction, length, evt, forceClone,
					ignoreCellAt || source.isTreeRoot());
				
				// Removes treeRoot flag in clones
				if (source.isTreeRoot())
				{
					for (var i = 0; i < cells.length; i++)
					{
						if (cells[i].isTreeRoot())
						{
	...
	graph.graphHandler.getCells = function(initialCell)
	{
		...
		for (var i = 0; i < temp.length; i++)
		{
			if (isTreeCell(temp[i]))
			{
				...
				graph.traverse(temp[i], true, createTraverseTreeVerticesAndEdgesWrapperFunction(function(vertex, edge)
				{
...
//		this.images.push(img);
		...
		// Delete
		var img = mxUtils.createImage(window['getVpResourceFileCurrentDirectoryOrEmpty']() + 'plugins/trees/handle-move.png');
		img.setAttribute('title', 'Move Cell without Subtree');
		img.style.position = 'absolute';
		img.style.cursor = 'pointer';
		img.style.width = '14px';
		img.style.height = '14px';
		img.style.left = (state.x + state.width - 14) + 'px';
		img.style.top = (state.y + state.height - 14) + 'px';
...
//		);
	...
	var iconTolerance = 20;
        ...
        var validateShowTreeCellIconCellState = function(cellState) {
            if (cellState != null) {
                var cell = cellState.cell;
                
                if (cell != null &&
                    cell.isShowTreeCellIcon() === false) {
	    ...
	    mouseMove: function(sender, me)
	    {
				...
				(!graph.getModel().isVertex(tmp.cell) || !isTreeCell(me.getCell()))))
			{
				tmp = null;
                        ...
                        tmp = validateShowTreeCellIconCellState(tmp);
