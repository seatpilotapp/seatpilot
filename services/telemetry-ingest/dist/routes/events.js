...
import { APP_CHANNEL, PLATFORM, CONFIRM_MODE, STATUS_EXIT, REASON_REASSIGN, assertEnum } from '@seatpilot/domain-model';
...
function getKeyForTenant(keyId) {
...
export async function ingestHandler(req, res) {
    try {
        ...
        if (!timingSafeEqualHex(sigHeader, expectedSig)) {
            await rejectToDLQ('signature_invalid', req.body, req.headers);
        ...
        for (const k of required) {
            if (body[k] === undefined) {
                await rejectToDLQ('envelope_missing', body, req.headers);
                return res.status(422).json({ error: 'envelope_missing', field: k });
            }
        }
        const handleInvalidEnum = async (error) => {
            const field = error?.field ?? 'unknown';
            await rejectToDLQ('invalid_enum', { error: error?.message, field, body }, req.headers);
        ...
        await rejectToDLQ('server_error', { err: e?.message }, req.headers);
