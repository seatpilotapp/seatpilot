...
export type HeadersRecord = Record<string, string | number | undefined>;

type SupabaseSchemaClient = ReturnType<SupabaseClient['schema']>;
type SupabaseActionResult = PostgrestSingleResponse<unknown>;

interface StoreAdapter {
  upsertEventsRaw(row: EventRow): Promise<void>;
  insertDLQ(reason: string, raw: unknown, headers: HeadersRecord): Promise<void>;
  trackAliasUse?(legacy: string, canonical: string, tenant: string): Promise<void>;
  resetEvents?(): void;
  resetDlq?(): void;
  resetAliasUsage?(): void;
  getEvents?(): EventRow[];
  getDlq?(): Array<{ reason: string; raw: unknown; headers: HeadersRecord }>;
  getAliasUsage?(): Array<{ legacy: string; canonical: string; tenant: string }>;
}

let supabase: SupabaseClient | null = null;
let supabaseTelemetry: SupabaseSchemaClient | null = null;
...
const memoryStore = (() => {
  ...
  const dlq: Array<{ reason: string; raw: unknown; headers: HeadersRecord }> = [];

  const aliasUsage: Array<{ legacy: string; canonical: string; tenant: string }> = [];

  const adapter: StoreAdapter = {
    async upsertEventsRaw(row: EventRow) {
      const key = row.event_id
        ? `${row.tenant_id}:${row.event_id}`
        : `${row.tenant_id}:${row.session_id}:${row.event_name}:${row.ts_ms}`;
      events.set(key, row);
    },
    async insertDLQ(reason: string, raw: unknown, headers: HeadersRecord) {
      dlq.push({ reason, raw, headers });
    },
    async trackAliasUse(legacy: string, canonical: string, tenant: string) {
...
const pgStore: StoreAdapter | null = pgPool
  ? {
      async upsertEventsRaw(row: EventRow) {
        const query = `
...
const supabaseStore: StoreAdapter | null = supabaseTelemetry
  ? {
      async upsertEventsRaw(row: EventRow) {
        const { error } = await withTimeout<SupabaseActionResult>(
          supabaseTelemetry!
            .from('events_raw')
            .upsert(
              {
      ...
      async insertDLQ(reason: string, raw: unknown, headers: HeadersRecord) {
        const { error } = await withTimeout<SupabaseActionResult>(
          supabaseTelemetry!.from('events_dlq').insert({ reason, raw, headers }),
          3000
        );
        if (error) throw error;
      },
...
let adapter: StoreAdapter = pgStore ?? supabaseStore ?? memoryStore;

export function configureStoreAdapter(override: StoreAdapter | null) {
  adapter = override ?? (pgStore ?? supabaseStore ?? memoryStore);
}

export async function upsertEventsRaw(row: EventRow) {
  await adapter.upsertEventsRaw(row);
}

export async function insertDLQ(reason: string, raw: unknown, headers: HeadersRecord) {
...
export async function getDlqCount(): Promise<number> {
  ...
  if (pgPool) {
    const res = await withTimeout(
      pgPool.query<{ n: number }>('select count(*)::int as n from telemetry.events_dlq'),
      2000
    );
